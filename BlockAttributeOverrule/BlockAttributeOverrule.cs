...
/// 
/// Revisions: Added a second overload of the Update()
/// method that takes all qualifying attributes as an
/// argument.
...
namespace AcMgdLib.DatabaseServices
{
   ...
   public abstract class BlockAttributeOverrule : ObjectOverrule<BlockReference>
   {
      ...
      /// </summary>
      /// <param name="tr">The transaction which the Attributes are
      /// opened with.</param>
      /// <param name="att">The AttributeReference to be operated on</param>
      /// <param name="owner">The owning BlockReference</param>

      protected virtual void Update(Transaction tr, AttributeReference att, BlockReference owner)
      {
      }
      ...
      bool IsMatch(AttributeReference att)
      {
         return !att.IsConstant && (noTags || tags.Contains(att.Tag));
      }
      ...
      public sealed override void Close(DBObject obj)
      {
         try
         {
               ...
               && IsMatch(blkref))
            {
               ...
               var map = new Dictionary<string, AttributeReference>(
                  attributes.Count, StringComparer.OrdinalIgnoreCase);
               ...
               try
               {
                  foreach(ObjectId id in attributes)
                  {
                     ...
                     if(IsMatch(attref))
                     {
                        ...
                        try
                        {
                           map.Add(attref.Tag, attref);
                           Update(tr, attref, blkref);
                        }
      ...
      /// REVISION:
      /// 
      /// This overridable method is called after all matching 
      /// AttributeReferences have been opened for write. The 
      /// attributes argument contains the AttributeReferences 
      /// keyed to their tags.
      /// 
      /// This method can be overridden in lieu of the overload
      /// taking a single attribute as its argument, to access 
      /// all of the block reference's matching attributes, that
      /// are write-enabled.
      /// </summary>
      /// <param name="owner"></param>
      /// <param name="attributes"></param>
      
      protected virtual void Update(Transaction trans,
